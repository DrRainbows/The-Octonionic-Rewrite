#!/usr/bin/env python3
"""Build book.html — a self-contained reader that opens directly in Chrome."""
import json, os
...
CHAPTERS = [
    ...
    ('ch10', '10', 'Universal Enveloping Algebras over O', 'book/part2-derivations/ch10-universal-enveloping-algebras-over-O.md', 'Part II: Derivations'),
...
content_map = {}
for cid, num, title, path, part in CHAPTERS:
    fpath = os.path.join(ROOT, path)
    with open(fpath, encoding='utf-8') as f:
        content_map[cid] = f.read()

# Build the chapters JS array (same format as index.html)
chapters_js = json.dumps(
    [{'id': c[0], 'num': c[1], 'title': c[2], 'part': c[4]} for c in CHAPTERS],
    ensure_ascii=False
)

# Build the content map as JSON
content_js = json.dumps(content_map, ensure_ascii=False)
...
<style>
:root{--bg:#fafaf8;--text:#2d2d2d;--text-light:#666;--accent:#2c5282;--border:#e0e0d8;--sidebar-bg:#f4f3ee;--sidebar-w:300px;--header-h:52px;--max-w:780px}
*{margin:0;padding:0;box-sizing:border-box}
html{font-size:16px;scroll-behavior:smooth}
body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:var(--bg);color:var(--text)}
header{position:fixed;top:0;left:0;right:0;height:var(--header-h);display:flex;align-items:center;padding:0 1rem;background:#fff;border-bottom:1px solid var(--border);z-index:200;gap:0.5rem}
header .title{font-weight:700;font-size:1rem;letter-spacing:-0.02em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;flex:1}
#menu-btn{display:none;background:none;border:none;font-size:1.4rem;cursor:pointer;padding:0.2rem 0.4rem;color:var(--text);line-height:1}
#print-btn{background:none;border:1px solid var(--border);border-radius:4px;padding:0.3rem 0.7rem;font-size:0.8rem;cursor:pointer;color:var(--text-light);white-space:nowrap}
#print-btn:hover{background:#f0f0ea;color:var(--text)}
...
/* Print-all mode */
.print-mode #sidebar,.print-mode #shade,.print-mode #pager,.print-mode #menu-btn,.print-mode #print-btn{display:none!important}
.print-mode header{position:static;border:none}
.print-mode #reader{margin-left:0}
.print-mode #chapter{max-width:100%;padding:0 1.5rem}
.print-mode .chapter-section{margin-bottom:3rem;padding-bottom:2rem;border-bottom:2px solid var(--border)}
.print-mode .chapter-section:last-child{border-bottom:none}
.print-progress{position:fixed;bottom:0;left:0;right:0;background:#fff;border-top:1px solid var(--border);padding:0.7rem 1.5rem;font-size:0.9rem;color:var(--text-light);z-index:300;display:flex;align-items:center;gap:1rem}
.print-progress .bar{flex:1;height:6px;background:#eee;border-radius:3px;overflow:hidden}
.print-progress .bar-fill{height:100%;background:var(--accent);border-radius:3px;transition:width 0.3s}
.print-progress .done-btn{background:var(--accent);color:#fff;border:none;padding:0.4rem 1rem;border-radius:4px;cursor:pointer;font-size:0.85rem;display:none}
...
@media print{
  header,#sidebar,#shade,#pager,#menu-btn,#print-btn,.print-progress{display:none!important}
  #reader{margin-left:0}
  #chapter{max-width:100%;padding:0;font-size:11pt;line-height:1.6}
  .chapter-section{page-break-before:always}
  .chapter-section:first-child{page-break-before:auto}
  .thm-block,.lem-block,.def-block,.example-block,.remark-block,.exercise-block{break-inside:avoid}
}
</style>
</head>
<body>
<header>
  ...
  <button id="print-btn">Print All / PDF</button>
</header>
<div id="layout">
  <nav id="sidebar"></nav>
  <div id="shade"></div>
...
function renderMd(md) {
  var codeSaved = [], mathSaved = [], ci = 0, mi = 0;
  ...
  for (var k = 0; k < codeSaved.length; k++) md = md.split(PFX+'C'+k+'_').join(codeSaved[k]);
  var html = marked.parse(md);
  for (var k = 0; k < mathSaved.length; k++) html = html.split(PFX+'M'+k+'_').join(mathSaved[k]);
...
function buildSidebar() {
  ...
  groups.forEach(function(part) {
    ...
    seen[part].forEach(function(ch) {
      out += '<a href="#' + ch.id + '" class="nav-ch" data-id="' + ch.id + '">' + (ch.num ? ch.num + '. ' : '') + ch.title + '</a>';
    });
    out += '</div></div>';
  });
...
function updatePager(ch) {
  ...
  if (idx > 0) {
    ...
    html += '<a href="#' + prev.id + '" class="pager-btn pager-prev">\u2190 ' + (prev.num ? prev.num + '. ' : '') + prev.title + '</a>';
  }
  if (idx < CHAPTERS.length - 1) {
    var next = CHAPTERS[idx + 1];
    html += '<a href="#' + next.id + '" class="pager-btn pager-next">' + (next.num ? next.num + '. ' : '') + next.title + ' \u2192</a>';
  }
  document.getElementById('pager').innerHTML = html;
}

/* ── Chapter Loading (from embedded data) ── */
var renderCache = {};
...
function showLanding() {
  ...
  document.getElementById('chapter').innerHTML =
    ...
    '<p class="sub">Hierarchical Octonionic Rewrite \u2014 Division and Reunification of Algebras</p>' +
...
async function loadChapter(ch) {
  ...
  var html;
  if (renderCache[ch.id]) {
    html = renderCache[ch.id];
  } else {
    var md = BOOK_CONTENT[ch.id];
    if (!md) { el.innerHTML = '<p class="error">Content not found.</p>'; return; }
    html = renderMd(md);
    renderCache[ch.id] = html;
  }
  el.innerHTML = html;
  setActive(ch.id);
  updatePager(ch);
...
async function printAll() {
  ...
  prog.className = 'print-progress';
  prog.innerHTML = '<span class="prog-text">Rendering...</span><div class="bar"><div class="bar-fill" style="width:0%"></div></div><button class="done-btn">Ready \u2014 Print Now</button>';
  document.body.appendChild(prog);
  var barFill = prog.querySelector('.bar-fill');
  var progText = prog.querySelector('.prog-text');
  var doneBtn = prog.querySelector('.done-btn');
  ...
  for (var i = 0; i < total; i++) {
    ...
    var md = BOOK_CONTENT[ch.id];
    if (!md) continue;
    var html = renderCache[ch.id] || renderMd(md);
    renderCache[ch.id] = html;

    // Add part header if first chapter in a new part
    if (i === 0 || CHAPTERS[i].part !== CHAPTERS[i - 1].part) {
      var partDiv = document.createElement('div');
      partDiv.className = 'chapter-section';
      partDiv.style.borderBottom = 'none';
      partDiv.style.textAlign = 'center';
      partDiv.style.padding = '2rem 0 0';
      partDiv.innerHTML = '<h1 style="font-family:-apple-system,sans-serif;font-size:1.6rem;color:#888">' + ch.part + '</h1>';
      el.appendChild(partDiv);
    }

    var section = document.createElement('div');
    section.className = 'chapter-section';
    section.id = 'print-' + ch.id;
    section.innerHTML = html;
    el.appendChild(section);

    var pct = Math.round(((i + 1) / total) * 70);
    barFill.style.width = pct + '%';
    progText.textContent = 'Rendering ' + (i + 1) + ' / ' + total + '...';
    // Yield to UI
    await new Promise(function(r){ setTimeout(r, 5); });
  }

  // MathJax typeset
  progText.textContent = 'Typesetting equations...';
  ...
  progText.textContent = 'Done. Use File \u2192 Print \u2192 Save as PDF.';
...
# Inject data
output = TEMPLATE.replace('%%CHAPTERS_JSON%%', chapters_js)
output = output.replace('%%CONTENT_JSON%%', content_js)

out_path = os.path.join(ROOT, 'book.html')
with open(out_path, 'w', encoding='utf-8') as f:
    f.write(output)

size_kb = os.path.getsize(out_path) / 1024
print(f"Built book.html ({size_kb:.0f} KB)")
